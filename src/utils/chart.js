...
export const getChart = (theme) => {
  ...
  Chart.defaults.scale.gridLines.color =
    theme === 'dark' ? '#4d4d4d' : 'rgba(233, 236 ,239, 1)';
  ...
  Chart.defaults.global.tooltips.custom = function customTooltip(tooltip) {
    let tooltipEl = document.getElementsByClassName(
  ...
  Chart.controllers.LineWithLine = Chart.controllers.line.extend({
    draw(ease) {
      ...
      if (this.chart.tooltip._active && this.chart.tooltip._active.length) {
        ...
        ctx.lineWidth = 0.5;
        ctx.strokeStyle = theme = 'dark' ? '#b3b3b3' : '#ddd';
        ctx.stroke();
        ctx.restore();
      }
    },
