...
const TemplateModal = ({ doc, setLoading, onFinish }) => {
  ...
  const onConfirm = async (val) => {
    try {
      ...
      await updateFirestore('sections/milk/template', doc._id, saveItem);
      // Add log.
      const log = {
        action: 'EDIT',
        module: 'TEMPLATE',
  ...
  const confirmDelete = async () => {
    try {
      setLoading(true);
      await updateFirestore('sections/milk/template', doc._id, {
        deleted: true,
        deleteBy: USER.uid,
      });
      // Add log.
