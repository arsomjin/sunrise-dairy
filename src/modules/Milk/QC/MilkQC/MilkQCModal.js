...
const MilkQCModal = ({ doc, setLoading, onFinish }) => {
  ...
  const onConfirm = async (val) => {
    try {
      ...
      await updateFirestore('sections/milk/milkQC', doc._id, saveItem);
      // Add log.
      const log = {
        action: 'EDIT',
        module: 'MILK_QC',
        command: 'MILK_QC_EDIT',
        docId: doc._id,
        snap: saveItem,
      };
      await addLogs(log);
  ...
  const confirmDelete = async () => {
    try {
      setLoading(true);
      await updateFirestore('sections/milk/milkQC', doc._id, {
        deleted: true,
        deleteBy: USER.uid,
      });
      // Add log.
