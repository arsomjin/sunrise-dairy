...
import { getMilkQCColumns } from './api';
...
import { renderMilkQCBody } from './components';
import MilkQCModal from './MilkQCModal';
...
const MilkQC = () => {
  ...
  const preFinish = async (val) => {
    try {
      ...
      const hasDup = await checkDuplicatedDoc('sections/milk/milkQC', {
  ...
  const onConfirm = async (val) => {
    try {
      ...
      const res = await addFirestore('sections/milk/milkQC', saveItem);
      // Add log.
      const log = {
        action: 'ADD',
        module: 'MILK_QC',
  ...
  return (
    <Page title="การตรวจคุณภาพน้ำนมดิบ" subtitle="รุ่งอรุณ แดรี่">
      <span className="mx-2 text-muted">วันที่</span>
      <DatePicker value={date} onChange={onDateChange} />
      <div className="mt-3">
        <EditableCellTable
          dataSource={mData}
          columns={getMilkQCColumns(mData)}
          loading={loading}
