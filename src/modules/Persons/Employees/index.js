...
import { getEmployeesColumns } from './api';
...
const Employees = () => {
  ...
  const getEmployees = useCallback(
    async (sDate) => {
      try {
        setLoading(true);
        const res = await getFirestoreCollection('employees');
        const arr = res
  ...
  const preFinish = async (val) => {
    try {
      ...
      const hasDup = await checkDuplicatedDoc('employees', {
  ...
  const onConfirm = async (val) => {
    try {
      ...
      const res = await addFirestore('employees', saveItem);
      // Add log.
      const log = {
        action: 'ADD',
        module: 'USERS',
  ...
  return (
    <Page title="รายชื่อผู้ใช้งาน" subtitle="รุ่งอรุณ แดรี่">
      <div className="mt-3">
        <EditableCellTable
          dataSource={mData}
          columns={getEmployeesColumns(mData)}
          loading={loading}
      ...
      {addable && (
        <Form form={form} onFinish={preFinish} scrollToFirstError>
          {(values) => {
            //   showLog({ values });
            return renderEmployeesBody({
              mobileOnly,
              prefixSelector,
              t,
              onClear,
